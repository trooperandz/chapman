<?php
/*-----------------------------------------Set report variables-----------------------------------------------*/
$chartarraytitle1 = 'SI Model Age'							;	// Set title to appear in chart legend
$chartarraytitle2 = '% SI Model Age'						;	// Set title to appear in chart legend
$chart_div		  = 'ymsichart'								;   // Set chart div name
$chart_border	  = '#FFFFFF'								;   // Set piechart slice border color
$chart_callback   = 'drawymsichart'							;   // Set chart callback variable
$chart_height	  = '600'									;	// Set pie area height
$chart_areaheight = '100%'									;   // Set chart area
$chart_areawidth  = '100%'									;   // Set chart area width
$chart_arealeft	  = '0'										;   // Set left chart area distance from border
$chart_top		  = '5'										;	// Set chart distance from chart area top
$chart_title	  = 'Single Issue Model Age Distribution'	;	// Set top report title
$chart_fontsize	  = '18'									;	// Set chart font size
$exportanchor  	  = 'ym_g_si_x.php'							;	// Set export anchor reference
$printeranchor 	  = 'ym_g_si_p.php'							; 	// Set printer friendly anchor reference
$tableid		  = 'ymsitable'								;   // Set table id for tablesorter functionality
$tabletitle 	  = 'Single Issue Model Age Data'			;	// Set table title
$tablehead1 	  = 'Category'								;	// Set first table header title
$tablehead2 	  = 'Total SI ROs'							;	// Set second table header title
$tablehead3 	  = 'Percentage'							;	// Set third table header title