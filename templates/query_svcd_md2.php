...
// The following two queries below together compute the total # of ROs that have services 1 through 9
$query = "SELECT DISTINCT ronumber FROM servicerendered 
WHERE serviceID IN ($all_svcs) AND addsvc = 0 AND dealerID IN($dealerIDs2) AND surveyindex_id IN ($surveyindex_id)";
...
$query = "SELECT DISTINCT ronumber FROM servicerendered 
WHERE serviceID IN ($L2_L3_svcs) AND addsvc = 0 AND dealerID IN($dealerIDs2) AND surveyindex_id IN ($surveyindex_id)";
$result = $mysqli->query($query);
if (!$result) { $_SESSION['error'][] = "Service Demand query failed.  See administrator.";
die (header("Location: enterrofoundation.php"));}
...
$query = "SELECT DISTINCT ronumber FROM servicerendered 
WHERE serviceID IN ($L2_svcs) AND dealerID IN($dealerIDs2) AND surveyindex_id IN ($surveyindex_id)";
$result = $mysqli->query($query);
if (!$result) { $_SESSION['error'][] = "Service Demand query failed.  See administrator.";
die (header("Location: enterrofoundation.php"));}
...
$query = "SELECT DISTINCT ronumber FROM servicerendered WHERE serviceID IN ($L3_svcs) AND addsvc = 0 AND dealerID IN ($dealerIDs2) AND surveyindex_id IN ($surveyindex_id)";
$result = $mysqli->query($query);
...
// If $L3_list is not null, run the query.  Else set $total_level2b2 = 0.
if ($L3_list != '') {
	$query = "SELECT DISTINCT ronumber FROM servicerendered WHERE serviceID IN ($L2_svcs) AND ronumber IN
			($L3_list)
			AND dealerID IN($dealerIDs2) and surveyindex_id IN ($surveyindex_id)";
	$result = $mysqli->query($query);
