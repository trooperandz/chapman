$(document).ready(function() {
	...
	$('body').on('click', '#metrics_dlr_comp_submit', function (event) {
		...
		if (date1_pres == false && date2_pres == false && dealer_group == false && all_dealers_checkbox == false) {
			getAlertModal('<p>You did not make a form selection!</p>');
			return false;
		}
	...
	$('body').on('click', '#metrics_search_submit', function (event) {
		...
		// Get advisor values. Note that for dealer non-admin users, this field will not be available(test for false)
		var advisor1 = document.getElementById('metrics_advisor1');
		var advisor1_id;
		var advisor1_label = "";
		if(advisor1) {
			advisor1       = advisor1.value.split(",");
			advisor1_id    = advisor1[0];
			advisor1_label = advisor1[1];
		} else {
			advisor1_id = false;
			advisor1_label = false;
		}
		...
		// Get region values. Test for null values first, and do not run value() or split() methods if null
		var region1 = document.getElementById('metrics_region1');
		var region1_id;
		var region1_label = "";
		if(region1) {
			region1       = region1.value.split(",");
			region1_id    = region1[0];
			region1_label = region1[1];
		} else {
			region1_id = false;
			region1_label = false;
		}

		// Get area values. Test for null values first, and do not run value() or split() methods if null
		var area1 = document.getElementById('metrics_area1');
		var area1_id;
		var area1_label = "";
		if(area1) {
			area1       = area1.value.split(",");
			area1_id    = area1[0];
			area1_label = area1[1];
		} else {
			metrics_area1_id = false;
			metrics_area1_label = false;
		}

		// Get district values. Test for null values first, and do not run value() or split() methods if null
		var district1 = document.getElementById('metrics_district1');
		var district1_id;
		var district1_label = "";
		if(district1) {
			district1       = district1.value.split(",");
			district1_id    = district1[0];
			district1_label = district1[1];
		} else {
			district1_id = false;
			district1_label = false;
		}
		...
		// If no selection was made at all, do not submit the form
		if (metrics_date1_pres == false && metrics_date2_pres == false && advisor1_id == false && region1_id == false && area1_id == false && district1_id == false && dealer_group == false ) {
            getAlertModal('<p>You did not make a form selection!</p>');
			return false;
		}
	...
	$('body').on('click', '#stats_search_submit', function (event) {
		...
		// Get advisor values. Note that for dealer non-admin users, this field will not be available(test for false)
		var advisor1 = document.getElementById('stats_advisor1');
		var advisor1_id;
		var advisor1_label = "";
		if(advisor1) {
			advisor1       = advisor1.value.split(",");
		...
		// Get region values. Test for null values first, and do not run value() or split() methods if null
		var region1 = document.getElementById('stats_region1');
		var region1_id;
		var region1_label = "";
		if(region1) {
			region1       = region1.value.split(",");
		...
		// Get area values. Test for null values first, and do not run value() or split() methods if null
		var area1 = document.getElementById('stats_area1');
		var area1_id;
		var area1_label = "";
		if(area1) {
			area1       = area1.value.split(",");
		...
		// Get district values. Test for null values first, and do not run value() or split() methods if null
		var district1 = document.getElementById('stats_district1');
		var district1_id;
		var district1_label = "";
		if(district1) {
			district1       = district1.value.split(",");
		...
		**/
		if (date1_pres == false && date2_pres == false && advisor1_id == false &&
			...
			dealer_group == false ) {
			getAlertModal('You must select at least one filter!');
			return false;
		}
	...
	$('body').on('click', '#metrics_trend_submit', function (event) {
		...
		console.log("advisor1 length: " + advisor1.length);
		//console.log('advisor1_id: ' + advisor1_id)
		//console.log('advisor1_label: ' + advisor1_label);

		// Get region values
		var region1 = document.getElementById('metrics_trend_region1');
		if(region1 != null) {
			console.log("region1 is something other than null");
			region1 = region1.value.split(",");
		...
		// Get area values
		var area1 = document.getElementById('metrics_trend_area1');
		if(area1 != null) {
			area1 = area1.value.split(",");
		...
		// Get district values
		var district1 = document.getElementById('metrics_trend_district1');
		if(district1 != null) {
			district1 = district1.value.split(",");
		...
		if(errors.length > 0 ){
			...
			getAlertModal(error_msg);
			return false;
		}

		// Test the getSearchFeedbackMsg() function.  Note: dateRequired needs to be set to true if user is trending dealer data
		var search_feedback = getSearchFeedbackMsg({
			dateRequired: true,
			date1_pres: date1_pres, date2_pres: date2_pres,
			advisor1: advisor1,   advisor1_id: advisor1_id,   advisor1_label:   advisor1_label,
			region1: region1,   region1_id: region1_id,   region1_label: region1_label,
			area1: area1,   area1_id: area1_id,   area1_label: area1_label,
			district1: district1,  district1_id: district1_id, district1_label: district1_label,
			dealer_group: dealer_group,  dlr_group_code: dlr_group_code,
			all_dealers_checkbox: all_dealers_checkbox
		});

		// If an error occurred in the feedback function, stop script execution
		if(!search_feedback) {
			return false;
		}

		//console.log('search_feedback from test function: ' + search_feedback);
		...
		var Data = { search_feedback:search_feedback, date1_pres:date1_pres, date2_pres:date2_pres,
					 advisor_id:advisor1_id, advisor_name:advisor1_label,
					 region_id:region1_id, region_name:region1_label, region1_id,
					 area_id:area1_id, area_name:area1_label, all_dealers_checkbox:all_dealers_checkbox,
	...
	$('body').on('click', '#ro_search_submit', function (event) {
        ...
        if (ro_num1 == false && ro_num2 == false && ro_date1 == false && ro_date2 == false && year1_id == false
            && year2_id == false && mileage1_id == false && mileage2_id == false && labor1 == false && labor2 == false
            && parts1 == false && parts2 == false && vehicle1_id == false && advisor1_id == false
            && svc_reg == false && svc_add == false && svc_dec == false && svc_exclude == false) {
            alert('You did not enter any filter criteria!');
            return false;
        }
	...
	$('body').on('click', '#add_dealer_submit', function (event) {
		...
		**/
		...
		if (errors.length > 0) {
			var error_msg = "<p>Your input contains the following errors:</p>";
			for (var i=0; i<errors.length; i++) {
				error_msg += "<p>" + errors[i] + "</p>";
			}
			error_msg += "<p>Please correct the errors and re-submit the form.</p>";
            getAlertModal(error_msg);
			return false;
		}
	...
	**/
	...
	$('body').on('click', 'input#file_submit', function (event) {
		...
		var error_msg = "<p>The following input errors have occurred: </p>";
		...
		if(errors.length > 0) {
			...
			error_msg += "<p>Please correct the errors and try again.</p>";
			//alert(error_msg);
            getAlertModal(error_msg);
			return false;
		}
	...
	// If SOS/Manuf user selects a dealer from the 'Select Dealer' dropdown list, change globals and serve metrics content
	$('select.dlr-select').on('change', function(event) {
		// Note: value includes the following, respectively: dealerID#dealername#dealercode
		var dlr = $(this).val();
		if(!dlr) {
			return false;
		} else {
			dlr = dlr.split('#');
			var dealer_id = dlr[0];
			var dealer_name = dlr[1];
			var dealer_code = dlr[2];
			console.log('dlr: ' + dlr);
		}

		var action = 'change_dealer_globals';
		...
		function ajaxCall() {
		 	$.ajax({
				type: "POST",
				url: processFile,
				data: 'action=' + action + '&dealer_id=' + dealer_id + '&dealer_name=' + dealer_name + '&dealer_code=' + dealer_code,
				success: function(returndata){
					console.log('returndata: ' + returndata);
					...
					// Re-initialize table functionality for all metrics tables
					$("#L1_table").DataTable({
					   paging: false,
					   searching: false,
					   order: []
    ...
    function getAlertModal(msg) {
        // Fill in the alert modal error message
        $('#alert-msg').html(msg);
        // Initialize and display the alert modal
        $('#alert-modal').foundation('reveal', 'open');
    }
});

// The following function builds the search feedback string
function getSearchFeedbackMsg(dataObj) {
	var search_feedback = "";

	console.log("dataObj.region1: " + dataObj.region1 + "typeof dataObj.region1: " + typeof dataObj.region1);

	if (dataObj.date1_pres != false && dataObj.date2_pres != false) {
		search_feedback += 'Date Range = ' + dataObj.date1_pres + ' - ' + dataObj.date2_pres + ' | ';
	}

	// Note: advisor1 will always be an object type, since the split() method is executed on it
	if (dataObj.advisor1 && dataObj.advisor1.length > 1) {
		search_feedback += 'Advisor = ' + dataObj.advisor1_label + ' | ';
	} else {
		dataObj.advisor1_id = false;
	}

	if (dataObj.region1 && dataObj.region1.length > 1) {
		search_feedback += 'Region = ' + dataObj.region1_label + ' | ';
	} else {
		dataObj.region1_id = false;
	}

	if (dataObj.area1 && dataObj.area1.length > 1) {
		search_feedback += 'Area = ' + dataObj.area1_label + ' | ';
	} else {
		dataObj.area1_id = false;
	}

	if (dataObj.district1 && dataObj.district1.length > 1) {
		search_feedback += 'District = ' + dataObj.district1_label + ' | ';
	} else {
		dataObj.district1_id = false;
	}

	// Build dealer code feedback comma-delimited list
	if (dataObj.dealer_group != false) {
		var dealer_group_feedback = '';
		for(i=0; i<dataObj.dealer_group.length; i++) {
			if(i == dataObj.dealer_group.length-1) {
				dealer_group_feedback += dataObj.dlr_group_code[i];
			} else {
				dealer_group_feedback += dataObj.dlr_group_code[i] + " - ";
			}
		}
	...
	if (dataObj.all_dealers_checkbox == true) {
		search_feedback += 'All Dealers | ';
	}

	// If there were

	// If user has submitted the Dealer Trending form, date is required. validate the dates and display error if necessary
	if (dataObj.dateRequired) {
		// If no date was selected, do not allow form submit (trending must always have a date range)
		if (dataObj.date1_pres == false && dataObj.date2_pres == false) {
			alert('You must enter a date range for trending!');
			return false;
		}
	}

	// If no selection was made at all, do not submit the form
	if (dataObj.date1_pres == false && dataObj.date2_pres == false && dataObj.advisor1_id == false &&
		dataObj.region1_id == false && dataObj.area1_id == false &&
		dataObj.district1_id == false ) {
