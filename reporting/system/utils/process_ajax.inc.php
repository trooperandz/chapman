...
if(verifyUserLoginAjax()) {
	...
	if (isset($actions[$_POST['action']])) {
		...
		if($_POST['action'] == 'enter_ros') {
			...
			if($_SESSION['user']['user_type_id'] == 1 || ($_SESSION['user']['user_type_id'] == 3 && $_SESSION['user']['user_admin'] == 1)) {
				echo $obj->$use_array['method2']();
			}
				echo $obj->$use_array['method3']($update_form = false, $update_ro_id = null, $search_params = false).
		...
		if($_POST['action'] == 'view_metrics_month' || $_POST['action'] == 'dealer_summary_select' || $_POST['action'] == 'change_dealer_globals') {
      ...
      $DealerInfo = new DealerInfo();
		...
		if($_POST['action'] == 'get_dealer_info_js') {
			// Make sure to make the result js-compatible
      $testObj = new DealerInfo($dbo = null);
      echo json_encode($testObj->getDealerListing());
		...
		**/
		if($_POST['action'] == 'view_dealer_users') {
			...
			echo $obj->$use_array['method1'](array('page_title'=>'Manage Users - ', 'title_info'=>$info, 'link_msg'=>'Add New User', 'a_id'=>'add_user_link', 'print-icon'=>true, 'export-icon'=>true, 'user_count'=>true)).
				 $user_table;
		}
		...
		if($_POST['action'] == 'view_manuf_users') {
			...
			echo $obj->$use_array['method1'](array('page_title'=>'Manage Users - ', 'title_info'=>MANUF.' Manufacturer Users', 'link_msg'=>'Add New User', 'a_id'=>'add_user_link', 'print-icon'=>true, 'export-icon'=>true, 'user_count'=>true)).
				 $user_table;
		}
		...
		if($_POST['action'] == 'add_doc_link') {
			echo $obj->$use_array['method1'](array('page_title'=>'System Documents - ', 'title_info'=>'Add New Document', 'a_id'=>null, 'link_msg'=>null)).
