...
Class Welr extends DB_Connect  {
	...
	// Note: Advisor dropdown should only be shown to SOS or admin users
	...
	public function insertRo($array) {
		...
		if(!($stmt->bind_param("isiiiiiddiisis", $array['ronumber'], $array['ro_date'], $array['yearmodelID'], $array['model_age'], $array['mileagespreadID'], $array['vehicle_make_id'], $array['singleissue'], $array['labor'], $array['parts'], $array['dealer_id'], $array['manuf_id'], $array['create_date'], $array['user_id'], $array['comment']))) {
			$error = "Binding parameters failed: (" . $stmt->errno . ") " . $stmt->error;
			sendError($error, __LINE__, __FILE__);
			$ro_insert = false;
			return 'error_ro_insert';
