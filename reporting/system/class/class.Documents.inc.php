...
Class Documents extends PDO_Connect {
	...
	/**
	 * Get documents data from database, for documents table
	 * @param {array} $array Array of specifications for determining which documents are loaded
	 * @return {array} $data Array of document data from database
	 */
	...
	/**
	 * Retrieve list of document categories for dropdown <option>'s. Save as SESSION var
	 * @param N/A
	 * @return [array] Array of document categories from database
	 */
	public function getDocCategories() {
		...
		// Manuf users do not have access to the 'Add New Document' menu link
		// SOS non-admin users should only be able to add to 'Release Forms' & 'My Documents' (no 'System Guides' access)
		if(!isset($_SESSION['doc_cat_list'])) {
			...
			// If user type == SOS non-admin, limit category to only 'Release Forms' & 'My Documents'
			if($this->user_sos && !$this->user_admin) {
				$stmt .= " WHERE doc_cat_id IN(1,3)";
			}
				...
				<div class="row">
					<div class="large-12 columns">
						...
						<table id="user_doc_table" class="original metric">
                            ...
                            // Provide edit doc and download doc icon html structure for reuse
                            $edit_doc_icon_html = '<a class="icon-document-edit tooltip-tip" title="Edit Doc" style="color: green;" name="edit_doc_icon">&nbsp;</a>';
							// Build html table body and export data rows based on increments set above
							for($i=0; $i<count($data); $i++) {
                                // Build delete doc icon html for reuse
                                $delete_doc_icon_html = '<a class="fontello-trash tooltip-tip" title="Delete Doc" style="color: #c7254e;" id="'.$data[$i]['tmp_name'].'" name="remove_doc_icon">&nbsp;</a>';
								// Format create_date to show just the date, not the time also
								$date = date("m/d/Y", strtotime($data[$i]['create_date']));
								$html .= '
								<tr>
									<td style="width: 86px;">';
                                    // Show edit icons based on user type && doc type
                                    if($this->user_sos && $this->user_admin) {
                                        $html .= $delete_doc_icon_html.$edit_doc_icon_html;
                                    } elseif ($this->user_sos && !$this->user_admin) {
                                        if($_SESSION['doc_type'] == 1 || $_SESSION['doc_type'] == 3) {
                                            $html .= $delete_doc_icon_html.$edit_doc_icon_html;
                                        }
                                    } elseif ($this->user_dlr && $this->user_admin) {
                                        if($_SESSION['doc_type'] == 3) {
                                            $html .= $delete_doc_icon_html.$edit_doc_icon_html;
                                        }
                                    }
									$html .='
